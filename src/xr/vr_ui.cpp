...
VrMenuItem menu_item_at(float x, float y) noexcept {
    ...
    constexpr float top=0.32f, height=0.125f, gap=0.025f;
    for(int i=0;i<3;++i) {
...
void render_pause_menu(std::span<std::uint8_t> p, std::uint32_t w, std::uint32_t h,
    std::uint32_t s, VrMenuItem selected, float px, float py, bool pointer_valid) {
    if(!valid(p,w,h,s)) return;
    ...
    const int t=static_cast<int>(h*8u/100u), b=static_cast<int>(h*88u/100u);
    ...
    constexpr std::array<std::string_view,3> labels{"RESUME","RESTART GAME","QUIT AREA51XR"};
    constexpr float top=0.32f, bh=0.125f, gap=0.025f;
    for(int i=0;i<3;++i) {
        ...
        centered(p,w,h,s,y0+(y1-y0-7*scale)/2,scale,labels[static_cast<std::size_t>(i)],white);
    }
    centered(p,w,h,s,static_cast<int>(h*81u/100u),std::max(1,scale-1),
